...
description: |
  ...
  活动列表、历史快照，以及按 product_id 查询理财产品历史。优先调用本机 FastAPI 后端。
argument-hint: [alpha / finance / activity / history / product_id]
...
- `GET /binance/finance`
  - 参数：
    - `sort_by=apr|term_days|product_name`
    - `order=asc|desc`
...
4. 活动走 `/binance/finance/activity`
5. 单产品历史优先带 `product_id`

## Examples

```bash
bash ~/.openclaw/skills/binance-alpha-finance/scripts/query.sh alpha
